# Aurora BG Toolkit — Optimization Journey (v9 → v17)
...
> recommendation.
>
> Total measurements across all versions: **377** over 10 days
> (2026-05-15 → 2026-05-24), ~$370 AWS spend, 0 production incidents.
...
v16       Instance × TPS matrix sweep (5月21-22日)          88 measurements   ⭐ STEVEN-GRADE
            ...
            └── ⚠️ RB ≈ 0ms reported, later proven a measurement blind-spot
v17       Reboot deep-dive 100 Hz (5月23-24日)             90 measurements   ⭐ FINAL
            └── 100 Hz STATS reporter (10× precision)       reveals true RB 10-200ms
            └── reader instance class is the dominant RB    HSK guidance: RB ≤ 30ms
                factor (t3.medium 190 / r7g.large 30 /         (with r7g.2xlarge reader)
                r7g.2xlarge 10-20)
```
...
## v17 — reboot deep-dive (100 Hz STATS)  ![](https://img.shields.io/badge/90%20measurements-validated)  ⭐

After v16 completed, audit of the raw logs revealed a disturbing inconsistency:
all 30 reboot measurements reported `writeMaxMs = 0ms` while v11-era logs for
the **same scenario** showed 10,359 wrapper events and 69 occurrences of
`write_ok=0`. v16 logs had only 17 non-STATS events (just startup + shutdown).

In a distributed system, reboot triggers cluster auto-failover, TCP reset,
in-flight transaction abort, and DNS rewrite. **A reboot that is 100% transparent
on the client side is physically impossible.** The "0ms" was almost certainly a
measurement artifact, not a real result.

### Root cause: 10 Hz STATS reporter sampling window math

```
v9-v16 STATS reporter: 10 Hz (sample every 100 ms)
  Real reboot gap (post-v17 measurement):
    r7g.2xlarge reader: 10-30 ms
    r7g.large reader:   30-50 ms
    t3.medium reader:   190-200 ms

If the gap (e.g. 25 ms) starts mid-window and ends in the same window,
both samples see write_ok still incrementing → writeMaxMs = 0.
Only gaps > 100ms reliably get caught.
```

### v17 instrumentation upgrade
...
These changes were achieved via YAML-only config updates
(`configs/v17-tps{1280,2560,4000}.yaml` set `wrapperLoggerLevel: FINER` and
`statsReporterHz: 100`). The Java code was already parameterized — no source
changes needed.

### Headline matrix (v17, 100 Hz, 90 measurements)
...
| smoke | r7g.large | t3.medium (1 cluster) | 1280 | **6620 ms** | — |

### Three findings that updated production guidance

1. **v16's "RB ≈ 0ms / transparent" was a measurement blind-spot.** The real
   number is **10-200 ms** depending on reader instance class. The mechanism
   (cluster auto-failover + JDBC wrapper transparent follow) is real and
   documented, but the latency is not zero.

2. **Reader instance class is the dominant factor for RB speed.** A 6× ladder:
   ```
   t3.medium reader      → RB 190 ms
   r7g.large reader      → RB  30 ms (6× faster)
   r7g.2xlarge reader    → RB  10-20 ms (1.5-3× faster, near 100Hz precision floor)
   ```
   Writer instance class has **no effect** on RB (M2 2X vs M3 4X vs M4 8X all
   converge to the same RB at the same reader spec).

3. **TPS does NOT affect RB.** The cluster auto-failover path is independent
   of the write workload. M4 1280 → T2 2560 → T3 4000 all stay 10-30 ms with
   the same r7g.2xlarge reader. **HSK can run at 4000 TPS production target
   without RB regression.**

### What v17 confirmed from v16

- ✅ v11-final.yaml stays the production recommendation (no client config changes)
- ✅ BG / FO numbers are stable (within ±25% across v16↔v17, no systematic shift)
- ✅ 8X r7g.8xlarge writer is comprehensively the best (still BG/FO best)
- ✅ T3 BG **5/5 success in v17** — v16's cluster-3/5 BG creation failures were
  occasional control-plane transients, not systemic. Production should still
  stagger BG switchovers, but the failure mode is rare not systemic

### What v17 corrected from v16

- ❌ "RB ≈ 0ms / transparent" → ✅ "RB 10-200ms by reader class"
- ❌ "reader is nice-to-have" → ✅ "reader class directly determines RB; r7g.2xlarge+ is HSK floor"
- ❌ "single-cluster topology untested" → ✅ "smoke run reproduced v11-era 6.6s degraded reboot, must NEVER use this topology"

### Wall time + cost

- 7 runs (smoke + M1-M4 + T2 + T3) total wall time: **~24 hours**
- AWS cost: **~$170** (same as v16 — same instance hours, just 100 Hz logs)
- 4 manual rescue cycles (cdk-destroy stuck on BG cleanup) were used to
  accelerate; would have been ~36 hours fully autonomous
- Operator time: ~30 minutes total (4 × ~7-min manual rescue cycles)

### Project totals after v17

- Total measurements: **377** (v9 120 + v10 30 + v11 25 + v12 24 + v16 88 + v17 90)
- Total AWS spend: **~$370**
- Versions formally tested: 7 (v9 / v10 / v11 / v12 / v16 / v17) + 3 exploratory
- Calendar days: 10 (2026-05-15 → 2026-05-24)
...
## Lessons that survived from v9 to v17

1. **Pre-register before measuring.** All formal experiments (v9, v11, v16, v17)
...
2. **Measurement precision must match measurement target.** v9-v16 used
   10 Hz STATS reporter (±100 ms precision). For BG / FO this is enough
   (gaps are seconds). For reboot (gap is 10-200 ms), 10 Hz is too coarse
   and produces false zeros — v17 had to upgrade to 100 Hz to get truth.
   **Always size precision 5-10× tighter than the smallest signal you want
   to catch.**

3. **0ms is a danger sign, not a victory.** When v16 reported RB ≈ 0ms
   across 30 measurements, the response should have been "this is too good
   to be true; investigate the measurement chain" rather than "great, we
   confirmed transparency". v17 took 24 hours and $170 to extract the truth
   that should have been suspected from day one.

4. **Reproducibility > one-off optimization.** v10's bash script could not
   answer "would this reproduce next week?". v11's CDK + Python orchestrator
   can. v17 reused v11's orchestrator with a config change and zero Java
   code changes. Future investments should bias toward reproducibility.

5. **Measure at production scale, not "small enough to fit in 30 min."**
   ...
   failures at 8X + 4000 TPS for the same reason. v17 found that reader
   class matters because it tested 4 reader classes.

6. **Document negative results as carefully as positive ones.** v12's
   ...
   v17's correction of v16 is just as valuable — it prevents future
   "the cluster auto-failover is transparent" claims that don't hold up.

7. **Honest reporting of partial failure.** v16 T3's 2/5 BG failure rate
   was a feature of the report, not a flaw. v17's correction of v16's
   "RB ≈ 0ms" claim is a feature, not an embarrassment — it shows the
   methodology worked (raw data was preserved enough to allow re-validation).

8. **Cross-reference multiple data sources.** v17 confirmed reboot really
   happened by combining: (a) client-side STATS (gap detected), (b)
   wrapper FINER log (failoverWriter event seen), (c) server-side
   describe-events (RDS reboot recorded). All three agreeing is robust;
   any one alone could be wrong.

---

*Last updated: 2026-05-25 (after v17 reboot deep-dive). Total measurement count tracked in `dashboard/`.

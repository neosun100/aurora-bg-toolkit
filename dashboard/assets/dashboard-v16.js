// dashboard-v16.js — v16 (Instance × TPS Matrix Sweep) dashboard logic.
// Loads dashboard/data/v16-only.json (produced by scripts/v16-extract-matrix.py
...
const SCENARIOS = [
  { key: 'blue-green', label: 'Blue/Green',   sub: 'T3: 3 cluster (cluster-3/5 failed @ 4000 TPS)',  color: 'var(--green)',  cls: 'green' },
  { key: 'failover',   label: 'Failover',     sub: 'T3: 5 clusters × 1 round = 5',  color: 'var(--orange)', cls: 'orange' },
  { key: 'reboot',     label: 'Reboot',       sub: 'T3: 5 measurements (cluster auto-failover)',   color: 'var(--cyan)',   cls: 'cyan' },
];
...
function verdict(scenarioKey, st) {
  ...
  const targets = {
    ...
    'reboot':     { ok: 100,   warn: 1000 },  // RB ≈ 0 (cluster auto-failover)
  };
...
async function load() {
  let data;
  try {
    const resp = await fetch('data/v16-only.json', { cache: 'no-store' });

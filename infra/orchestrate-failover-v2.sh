#!/usr/bin/env bash
# orchestrate-failover-v2.sh — high-load Failover round.
#
# Same workload as v2 BG (1280 ops/s, pool=50) but on existing GREEN clusters
# (post-switchover). Failover doesn't need a BG deployment; we can repeat
# 10 rounds on the same clusters.
...
ROUND_DIR="$REPO_ROOT/e2e-results/v2-failover-${ROUND}_$(date -u +%Y%m%d_%H%M%S)"
mkdir -p "$ROUND_DIR"
...
echo "============================================================"
echo " v2 Failover Round $ROUND  ($(date -u +%H:%M:%S) UTC)"
echo "============================================================"

ssh_ec2 "rm -rf /home/ec2-user/v2fo-${ROUND} && mkdir -p /home/ec2-user/v2fo-${ROUND}"
for c in test-02 test-03 test-04 test-05; do
    source "$STATE_DIR/${c}.env"
    cfg=${CFG[$c]}
    cat <<EOF | ssh_ec2 "bash -s" >/dev/null
mkdir -p /home/ec2-user/v2fo-${ROUND}/${c}
...
DB_ENDPOINT="$ABT_CLUSTER_ENDPOINT" DB_PORT=4488 DB_USER=admin DB_NAME=demo DB_PASSWORD="$TMP_PASSWORD" \\
  TABLE_SUFFIX="ec2_4_v2fo${ROUND}" WRAPPER_VERSION=4.0.0 \\
  nohup java --add-opens java.base/java.lang=ALL-UNNAMED --add-opens java.base/java.lang.reflect=ALL-UNNAMED \\
    ...
    > /home/ec2-user/v2fo-${ROUND}/${c}/ec2_wrapper4.log 2>&1 &
echo \$! > /home/ec2-user/v2fo-${ROUND}/${c}/ec2_wrapper4.pid
EOF
done
...
echo "[3] Triggering failover on 4 clusters..."
for c in test-02 test-03 test-04 test-05; do
...
for c in test-02 test-03 test-04 test-05; do
    pid=$(ssh_ec2 "cat /home/ec2-user/v2fo-${ROUND}/${c}/ec2_wrapper4.pid 2>/dev/null || echo 0")
...
for c in test-02 test-03 test-04 test-05; do
    ...
    scp -i "$ABT_KEY_FILE" -o StrictHostKeyChecking=no -o LogLevel=ERROR \
        ec2-user@"$ABT_EC2_PUBLIC_IP":/home/ec2-user/v2fo-${ROUND}/${c}/ec2_wrapper4.log \
        "$ROUND_DIR/${c}_${cfg}/ec2_wrapper4.log" 2>/dev/null
    cat > "$ROUND_DIR/${c}_${cfg}/meta.json" <<JSON
{"runId": "${c}_${cfg}_v2fo_r${ROUND}", "config": "$cfg", "scenario": "failover", "round": $ROUND}
JSON
...
echo "v2 Failover Round $ROUND results:"
for c in test-02 test-03 test-04 test-05; do
    cfg=${CFG[$c]}

#!/usr/bin/env bash
# launch-matrix.sh — One-shot v16 matrix sweep launcher.
#
...
#   3. Deploy AbtV16MatrixRunnerStack (creates t3.small runner EC2 + S3 + SNS)
...
log "Step 3/8: deploy AbtV16MatrixRunnerStack (t3.small + S3 + SNS)"
cd infra/cdk
...
RUNNER_IP="$(aws cloudformation describe-stacks --stack-name AbtV16MatrixRunnerStack \
             --query "Stacks[0].Outputs[?OutputKey=='RunnerPublicIp'].OutputValue" --output text)"
BUCKET="$(aws cloudformation describe-stacks --stack-name AbtV16MatrixRunnerStack \
          --query "Stacks[0].Outputs[?OutputKey=='StateBucketName'].OutputValue" --output text)"
TOPIC_ARN="$(aws cloudformation describe-stacks --stack-name AbtV16MatrixRunnerStack \
             --query "Stacks[0].Outputs[?OutputKey=='TopicArn'].OutputValue" --output text)"
DASHBOARD_URL="$(aws cloudformation describe-stacks --stack-name AbtV16MatrixRunnerStack \
...
# re-deployed (e.g. after a v15 destroy + v16 redeploy), in which case the
...
SERVICE_FILE="/tmp/abt-v16-matrix.service"
cat > "$SERVICE_FILE" << SVCEOF
[Unit]
Description=Aurora BG Toolkit v16 Matrix Sweep
...
Restart=on-failure
RestartSec=60
StandardOutput=append:/var/log/abt-v16-matrix.log
StandardError=append:/var/log/abt-v16-matrix.log
...
scp $SSH_OPTS "$SERVICE_FILE" ec2-user@"$RUNNER_IP":/tmp/abt-v16-matrix.service
ssh $SSH_OPTS ec2-user@"$RUNNER_IP" \
    "sudo mv /tmp/abt-v16-matrix.service /etc/systemd/system/ && \
     sudo chmod 0640 /etc/systemd/system/abt-v16-matrix.service && \
     sudo chown root:root /etc/systemd/system/abt-v16-matrix.service && \
     sudo systemctl daemon-reload && \
     sudo systemctl enable abt-v16-matrix.service"
rm -f "$SERVICE_FILE"
ok "Systemd service installed (with Bark credentials, mode 0640)"

# ── Step 8: Start the service ───────────────────────────────────
log "Step 8/8: start abt-v16-matrix.service on runner"
ssh $SSH_OPTS ec2-user@"$RUNNER_IP" \
    "sudo systemctl restart abt-v16-matrix.service && \
     sleep 3 && sudo systemctl status abt-v16-matrix.service --no-pager | head -15"
ok "Service started"
...
echo "  • Live log:          ssh ... 'sudo journalctl -u abt-v16-matrix -f'"
echo
echo "Notifications:"
...
echo "  ssh -i $KEY_PATH ec2-user@$RUNNER_IP 'sudo systemctl stop abt-v16-matrix'"
echo "  ssh -i $KEY_PATH ec2-user@$RUNNER_IP 'sudo systemctl restart abt-v16-matrix'"
echo

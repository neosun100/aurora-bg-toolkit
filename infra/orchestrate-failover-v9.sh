#!/usr/bin/env bash
# orchestrate-failover-v9.sh — single Failover round across same 4 cells.
set -euo pipefail
...
ROUND_DIR="$REPO_ROOT/e2e-results/v9-failover-${ROUND}_$(date -u +%Y%m%d_%H%M%S)"
mkdir -p "$ROUND_DIR"
...
echo "============================================================"
echo " v9 Failover Round $ROUND  ($(date -u +%H:%M:%S) UTC)"
echo "============================================================"

ssh_ec2 "rm -rf /home/ec2-user/v9fo-${ROUND} && mkdir -p /home/ec2-user/v9fo-${ROUND}"
for c in test-02 test-03 test-04 test-05; do
    source "$STATE_DIR/${c}.env"
    cfg=${CFG[$c]}; jar=${JAR[$c]}
    cat <<EOF | ssh_ec2 "bash -s" >/dev/null
mkdir -p /home/ec2-user/v9fo-${ROUND}/${c}
...
DB_ENDPOINT="$ABT_CLUSTER_ENDPOINT" DB_PORT=4488 DB_USER=admin DB_NAME=demo DB_PASSWORD="$TMP_PASSWORD" \\
  TABLE_SUFFIX="ec2_v9fo${ROUND}" WRAPPER_VERSION="${jar%.jar}" \\
  nohup java -Dnetworkaddress.cache.ttl=5 -Dnetworkaddress.cache.negative.ttl=2 \\
    ...
    > /home/ec2-user/v9fo-${ROUND}/${c}/ec2_wrapper.log 2>&1 &
echo \$! > /home/ec2-user/v9fo-${ROUND}/${c}/ec2_wrapper.pid
EOF
done
...
for c in test-02 test-03 test-04 test-05; do
    pid=$(ssh_ec2 "cat /home/ec2-user/v9fo-${ROUND}/${c}/ec2_wrapper.pid 2>/dev/null || echo 0")
...
for c in test-02 test-03 test-04 test-05; do
    ...
    scp -i "$ABT_KEY_FILE" -o StrictHostKeyChecking=no -o LogLevel=ERROR \
        ec2-user@"$ABT_EC2_PUBLIC_IP":/home/ec2-user/v9fo-${ROUND}/${c}/ec2_wrapper.log \
        "$ROUND_DIR/${c}_${cfg}/ec2_wrapper.log" 2>/dev/null
    cat > "$ROUND_DIR/${c}_${cfg}/meta.json" <<JSON
{"runId": "${c}_${cfg}_v9fo_r${ROUND}", "config": "$cfg", "scenario": "failover", "round": $ROUND, "wrapperJar": "${JAR[$c]}"}
JSON
...
echo "v9 Failover Round $ROUND results:"
for c in test-02 test-03 test-04 test-05; do
    cfg=${CFG[$c]}

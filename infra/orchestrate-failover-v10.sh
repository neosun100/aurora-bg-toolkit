#!/usr/bin/env bash
# orchestrate-failover-v10.sh — single Failover round for the v10 experiment.
#
# Single cell: cluster=test-v10, config=v10-final, wrapper=4.0.1.
#
# Usage: ./orchestrate-failover-v10.sh <round-number>
...
ROUND_DIR="$REPO_ROOT/e2e-results/v10-failover-${ROUND}_$(date -u +%Y%m%d_%H%M%S)"
...
echo "============================================================"
echo " v10 Failover Round $ROUND  ($(date -u +%H:%M:%S) UTC)"
echo "============================================================"

ssh_ec2 "rm -rf /home/ec2-user/v10fo-${ROUND} && mkdir -p /home/ec2-user/v10fo-${ROUND}"
cat <<EOF | ssh_ec2 "bash -s" >/dev/null
mkdir -p /home/ec2-user/v10fo-${ROUND}/test-v10
...
DB_ENDPOINT="$ABT_CLUSTER_ENDPOINT" DB_PORT=4488 DB_USER=admin DB_NAME=demo \\
  ...
  TABLE_SUFFIX="ec2_v10fo${ROUND}" WRAPPER_VERSION="${JAR%.jar}" \\
  nohup java -Dnetworkaddress.cache.ttl=5 -Dnetworkaddress.cache.negative.ttl=2 \\
    ...
    > /home/ec2-user/v10fo-${ROUND}/test-v10/ec2_wrapper.log 2>&1 &
echo \$! > /home/ec2-user/v10fo-${ROUND}/test-v10/ec2_wrapper.pid
EOF
...
echo "[3] Triggering failover on test-v10..."
aws_ rds failover-db-cluster --db-cluster-identifier "test-v10" >/dev/null
...
pid=$(ssh_ec2 "cat /home/ec2-user/v10fo-${ROUND}/test-v10/ec2_wrapper.pid 2>/dev/null || echo 0")
...
scp -i "$ABT_KEY_FILE" -o StrictHostKeyChecking=no -o LogLevel=ERROR \
    ec2-user@"$ABT_EC2_PUBLIC_IP":/home/ec2-user/v10fo-${ROUND}/test-v10/ec2_wrapper.log \
    "$ROUND_DIR/test-v10_${CFG}/ec2_wrapper.log" 2>/dev/null

cat > "$ROUND_DIR/test-v10_${CFG}/meta.json" <<JSON
{"runId": "test-v10_${CFG}_v10fo_r${ROUND}", "config": "$CFG", "scenario": "failover", "round": $ROUND, "wrapperJar": "${JAR}", "experiment": "v10-production"}
JSON
...
if [[ -f "$ROUND_DIR/test-v10_${CFG}/stats-gap.json" ]]; then
    ...
    echo
    echo "v10 Failover Round $ROUND result:"

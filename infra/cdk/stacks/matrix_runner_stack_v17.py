...
Resources:
  ...
  - 1 S3 bucket: abt-v17-state-{account}: holds progress.json, master.log,
    dashboard.html, per-run data JSONs, final report
  - 1 SNS topic: abt-v17-events: subscriber emails configured via env var
    ...
    def __init__(
        ...
        **kwargs,
        ...
        state_bucket = s3.Bucket(
            self, "AbtV17StateBucket",
            bucket_name=f"abt-v17-state-{self.account}",
        ...
        # ── 2. SNS topic ──
        topic = sns.Topic(
            self, "AbtV17EventsTopic",
            topic_name="abt-v17-events",
        ...
        user_data.add_commands(
            "set -e",
            "exec > /var/log/abt-v17-runner-userdata.log 2>&1",
            "yum update -y",
            # Core tooling
            ...
            "echo 'runner user-data complete: '$(date -u) >> /var/log/abt-v17-runner-userdata.log",
        )

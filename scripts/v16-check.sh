...
# orchestrator state — the source of truth is s3://abt-v16-state-{account}/.
#
# Usage:
...
if [[ -z "${ABT_STATE_BUCKET:-}" ]]; then
    ...
    ABT_STATE_BUCKET="abt-v16-state-$ACCOUNT_ID"
fi
...
print(f"{BLD}{'='*78}{NC}")
print(f"{BLD} Aurora BG Toolkit v16 — Matrix Sweep status{NC}")
print(f"{BLD}{'='*78}{NC}")
...
bucket = os.environ.get("BUCKET", "abt-v16-state")
